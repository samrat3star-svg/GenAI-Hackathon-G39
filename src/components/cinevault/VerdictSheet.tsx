...
const VERDICTS = [
  ...
  },
  {
    ...
    color: "bg-pink-500/10 text-pink-600 border-pink-500/30 hover:bg-pink-500/20",
  },
  {
    id: "life",
    label: "LIFE SENTENCE",
...
export function VerdictSheet({ open, movieTitle, onOpenChange, onPick }: VerdictSheetProps) {
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent side="bottom" className="bg-card backdrop-blur-xl border-t border-border rounded-t-3xl sm:max-w-xl sm:mx-auto p-6 md:p-8">
        <SheetHeader className="text-left mb-8">
          <div className="flex items-center justify-center w-12 h-12 rounded-full bg-secondary mb-4 border border-border">
            <Gavel className="w-6 h-6 text-primary" />
          </div>
          <SheetTitle className="font-display text-2xl md:text-3xl text-card-foreground">
            How did <span className="text-primary italic">{movieTitle}</span> land?
          </SheetTitle>
          <p className="text-muted-foreground text-sm">Select a verdict. This replaces star ratings.</p>
        </SheetHeader>
        ...
        <div className="grid gap-3 pb-8">
          {VERDICTS.map((v, i) => (
            <motion.button
              ...
              className={`flex flex-col items-start p-4 rounded-xl border transition-all duration-300 bg-secondary hover:bg-primary hover:text-primary-foreground border-border group ${v.color.split(' ').filter(c => !c.startsWith('bg-') && !c.startsWith('text-') && !c.startsWith('border-')).join(' ')}`}
            >
